...
TSharedPtr<FUnrealType> CreateUnrealTypeInfo(UStruct* Type, uint32 ParentChecksum, int32 StaticArrayIndex)
{
	...
	for (TFieldIterator<GDK_PROPERTY(Property)> It(Type); It; ++It)
	{
		...
		//CharacterMovementComponent) 		are in fact owned by the character, and can be stored in the same entity as the character itself. Some
		//subobjects (such as the Controller 		field in AActor) is a weak reference, and should just store a reference to the real object. We
		//inspect the CDO to determine whether 		the owner of the property value is equal to itself. As structs don't have CDOs, we assume
		//that all object properties in structs are 		weak references.
		//
